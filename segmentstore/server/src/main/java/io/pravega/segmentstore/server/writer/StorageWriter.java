...
class StorageWriter extends AbstractThreadPoolService implements Writer {
    ...
    private CompletableFuture<?> processOperation(Operation op) {
    ...
    private CompletableFuture<?> processMetadataOperation(MetadataOperation op) {
        ...
        } else if (op instanceof CreateSegmentOperation) {
            return createNewSegment((CreateSegmentOperation) op);
        }
    ...
    private CompletableFuture<Void> processStorageOperation(StorageOperation op) {
        ...
        return openExistingSegment(op.getStreamSegmentId())
    ...
    private CompletableFuture<SegmentAggregator> openExistingSegment(long streamSegmentId) {
        return getSegmentAggregator(streamSegmentId, sa -> sa.initialize(this.config.getFlushTimeout()));
    }

    private CompletableFuture<SegmentAggregator> createNewSegment(CreateSegmentOperation op) {
        return getSegmentAggregator(op.getStreamSegmentId(), sa -> sa.createNewSegment(this.config.getFlushTimeout()));
    }
     ...
     */
    private CompletableFuture<SegmentAggregator> getSegmentAggregator(long streamSegmentId, Function<SegmentAggregator, CompletableFuture<Void>> initializer) {
        ...
        val newAggregator = new SegmentAggregator(segmentMetadata, this.dataSource, this.storage, this.config, this.timer, this.executor);
        try {
            val init = initializer.apply(newAggregator);
